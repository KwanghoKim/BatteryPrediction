...
class Model:
  def __init__(self):
    ...
    self.target_categories = ["screen_usagePowerMah", \
        "wifiPowerMah", "cpuPowerMah", "cameraPowerMah",\
        "audioPowerMah", "videoPowerMah", "totalPowerMah", "all_cpuPowerMah", "all_totalPowerMah"]
    ...
    self.target_categories = ["audioTimeMs", "cpuTimeMs", "videoTimeMs", "screen_usageTimeMs",  "wifiRunningTimeMs", "all_cpuTimeMs"]
...
#@@        print("#!#!#!#!@@@@@@@@@@@@@ total_x_data=", total_x_data)
...
#@#@        print("#!#!#!#!#! @@@@@@@@ path=", path, "dir=", directories, "files=", files)
        ...
        **consider when more parameters are needed
        ...
        for _file in files:
          ...
          sum_cpuPowerMah = 0
          sum_cpuTimeMs = 0
#@#@          print("#!#!#!#!#! [init] sum_totalPowerMah = ", sum_totalPowerMah)
          if(_file.startswith(log)):
            ...
            for line in lines.split("\n"):
              try:
                ...
                if("totalPowerMah" in json_data.keys()):
#@#@                  print("json_data[totalPowerMah] =", json_data["totalPowerMah"])
                  sum_totalPowerMah += json_data["totalPowerMah"]
#@#@                  data["all_totalPowerMah"] = sum_totalPowerMah
#@#@                  print("#!#!#!#!#! sum_totalPowerMah = ", sum_totalPowerMah)

                if("cpuPowerMah" in json_data.keys()):
                  sum_cpuPowerMah += json_data["cpuPowerMah"]
#@#@                  data["all_cpuPowerMah"] = sum_cpuPowerMah 

                if("cpuTimeMs" in json_data.keys()):
                  sum_cpuTimeMs += json_data["cpuTimeMs"]

                #some data don't have target app power data:(
                if("com.gsrathoreniks.facefilter" in json_data.values()):
                  data = {**data, **json_data}
#@#@                  data["all_totalPowerMah"] = json_data["totalPowerMah"]
...
#!#!#!#!#!                if("totalPowerMah" in json_data.values()):
#!#!#!#!#!                print("#!#! @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ##004")
#!#!#!#!#!                data["all_totalPowerMah"] = json_data["totalPowerMah"]
              except Exception as e:
                pass
            data["all_totalPowerMah"] = sum_totalPowerMah
            data["all_cpuPowerMah"] = sum_cpuPowerMah 
            data["all_cpuTimeMs"] = sum_cpuTimeMs 
  ...
  def build_model(self):
    ...
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ size_x =", len(self.x_train[0]), "size_y =", len(self.y_train[0]))
#@#@#@#@    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ x_train[0] =", self.x_train[0], "y_train[0]=", self.y_train[0])
    ...
    with open('model_time_v1.0.tflite', 'wb') as f:
  ...
  def apply_data_regularization(self):
    ...
    stan =  (self.x_train - mean) / std
    self.x_train = stan

    mean = self.x_test.mean(axis= 0)
    std = self.x_test.std(axis=0)
    stan =  (self.x_test - mean) / std
    self.x_test = stan
    '''
    print("@@@@@@@@@@@@@ [apply_data_regularization](raw) x_test = ", self.x_test)
    ...
    #self.x_validate /= std
    self.x_test -= mean
    print("@@@@@@@@@@@@@ [apply_data_regularization](mean) x_test = ", self.x_test)
    self.x_test /=std
    print("@@@@@@@@@@@@@ [apply_data_regularization](std) x_test = ", self.x_test)
...
#@@    print("@@@@@@@@@@@@@@@[predict_power_per_time] x_test = ", self.x_test)
#@@    print("@@@@@@@@@@@@@@@[predict_power_per_time] y_pred = ", y_pred)
...
train_target_dirs =["iu", "kookyohwan"]
#@@train_target_dirs =["iu", "kookyohwan", "leejeongjae", "onebeen", "yunakim"]
#@@test_target_dirs =["parkjunghun"]
test_target_dirs =["parkjunghun_test"]
#@@test_target_dirs =["temp"]
#@@test_target_dirs =["test_dataset/onebeen"]
#@@test_target_dirs =["parkjunghum_max_freq/60s", "parkjunghum_max_freq/120s", "parkjunghum_max_freq/180s", "parkjunghum_max_freq/300s", "parkjunghum_max_freq/600s"]
#@@test_target_dirs =["backup_parkjunghum_max_freq/60s", "backup_parkjunghum_max_freq/120s", "backup_parkjunghum_max_freq/180s", "backup_parkjunghum_max_freq/300s", "backup_parkjunghum_max_freq/600s"]
#@@test_target_dirs =["parkjunghun_minfreq/60s", "parkjunghun_minfreq/120s", "parkjunghun_minfreq/180s", "parkjunghun_minfreq/300s", "parkjunghun_minfreq/600s"]
#@@test_target_dirs =["1st", "2nd"]

for train_target_dir in train_target_dirs:
  train_data.extend(model.parse_data_from_dir(os.path.join("logs/logs_0B131JEC200794/train_dataset", train_target_dir)))


for test_target_dir in test_target_dirs:
  test_data.extend(model.parse_data_from_dir(os.path.join("logs/logs_0B131JEC200794/test_dataset", test_target_dir)))

#@@for test_target_dir in test_target_dirs:
#@@  test_data.extend(model.parse_data_from_dir(os.path.join("logs/logs_0B131JEC200794/test_dataset/parkjunghun_cputhrottling", test_target_dir)))
