...
#@@filters = {"hair" : 600, "mask" : 600, "rabbit" : 600, "glasses" : 600, "hair" : 300, "mask" : 300, "rabbit" : 300, "glasses" : 300, "hair" : 180, "mask" : 180, "rabbit" : 180, "glasses" : 180, "hair" : 120, "mask" : 120, "rabbit" : 120, "glasses" : 120, "hair" : 60, "mask" : 60, "rabbit" : 60, "glasses" : 60} 
...
def launch_cputhrottling():
  exec("am start -n skynet.cputhrottlingtest/.MainActivity")

def quit_cputhrottling():
  exec("am force-stop skynet.cputhrottlingtest")

def set_cputhrottling(model):
  if(model == "pixel_4xl"):
    exec("sleep 3 && input tap 456 186 && sleep 0.5")
  elif(model == "pixel_4a"):
    exec("sleep 3 && input tap 294 175 && sleep 0.5")
  ...
  for _iter in range(1): 
    ...
    for filter_type, rand_time in filters.items():
      ...
      for i in range(1):
        ...
        quit_facefilter()
        disable_screen_off()
        launch_cputhrottling()
        reset()
        unplug()
        set_cputhrottling(args.model)
...
#@@        set_min_freq(args.model)
        ...
        #@@ add simpleperf
        measure_perf("skynet.cputhrottlingtest", rand_time, i, target)
